    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "https://pankhay.com/thewhitehousegame/public/api/voter_candidate_list",
          {
            headers: {
        ...
        setData(response.data.votter_candidate_list);
        setLoading(false);
      } catch (error) {
        setError(error);
        setLoading(false);
    ...
    <>
      <div className='customSlider'>
        ...
        <div className="candidatesList  w-10/12 m-auto mt-8 resp relative">
          {loading && <h1 className='text-[red]'>Loading...</h1>}
          {!extended ? <Slider {...settings}>
            {data?.map((item, index) => (
              <div key={index} className="candidate flex flex-col items-center justify-between gap-[10px] " onClick={() => handleCandidateClick(index)}>
                <div className="image w-[116px] h-[116px] rounded-[50%]  flex">
                  <img className=' w-[116px] h-[116px] rounded-[50%] object-cover' src={`${imageUrl}${item?.candidate_image}`} alt="" />
                </div>
                <h5 className='poppins5 text-[17px] text-blackColor'>{item.candidate_name}</h5>
              </div>
            ))}
            ...
            {data?.map((item, index) => (
              <div key={index} className="candidate flex flex-col items-center justify-between gap-[10px]">
                <div className="image w-[116px] h-[116px] rounded-[50%]  flex">
                  <img className=' w-[116px] h-[116px] rounded-[50%] object-fit' src={`${imageUrl}${item?.candidate_image}`} alt="" />
                </div>
                <h5 className='poppins5 text-[17px] text-blackColor'>{item.candidate_name}</h5>
              </div>
            ))}

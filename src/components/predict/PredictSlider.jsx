...
const CustomNextArrow = (props) => (
    <div
        {...props}
        className="absolute top-[40%] right-[0%]  cursor-pointer"
    >
...
function PredictSlider({ data, printData }) {
    ...
    return (
        <div className=' '>
            <Slider {...settings}>
                {data.map((data, index) => (
                    <div key={index} className='xl:w-[567.38px] xl:h-[572.84px] rounded-[28.43px] border-[10px] border-transparent overflow-hidden hover:border-[10px] hover:border-white'>
                        <img onClick={printData} className='w-full h-full object-cover' src={data} alt="" />
                    </div>
                ))}
            </Slider>
        </div>
